...
function updateAuthUi(user) {
  ...
  document.querySelectorAll(".nav-logout").forEach(function(element) {
    element.style.display = isAuthenticated ? "" : "none";
  });
...
function getFirebaseAuth() {
  ...
  try {
    ...
    console.error("Firebase Auth nao inicializado:", error);
    return null;
  }
}
...
function initializeFirebaseAuth() {
  ...
  if (auth.getRedirectResult) {
    auth.getRedirectResult().catch(function(error) {
      console.error("Erro ao concluir login com Google via redirect:", error);
    });
  }
}
...
async function signInWithGoogle() {
  ...
  if (!auth || !window.firebase || !window.firebase.auth || !window.firebase.auth.GoogleAuthProvider) {
    alert("Firebase nao configurado ainda. Preencha firebase-config.js com apiKey, authDomain, projectId e appId.");
    return;
  }
  ...
  try {
    ...
    alert("Nao foi possivel iniciar o login com Google.");
  }
}
...
if (googleAuthButton) {
  ...
  googleAuthButton.addEventListener("keydown", function(event) {
